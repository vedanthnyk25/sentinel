...
type GetAllInventoryRow struct {
	EventID          uuid.NullUUID `json:"event_id"`
	AvailableTickets int32         `json:"available_tickets"`
}

func (q *Queries) GetAllInventory(ctx context.Context) ([]GetAllInventoryRow, error) {
	rows, err := q.db.QueryContext(ctx, getAllInventory)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetAllInventoryRow
	for rows.Next() {
		var i GetAllInventoryRow
		if err := rows.Scan(&i.EventID, &i.AvailableTickets); err != nil {
			return nil, err
		}
